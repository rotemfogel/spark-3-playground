...
object UserAgentSparkApp extends ParquetReaderApplication {
  ...
  private def userAgentExtractColumns(col: Column, conf: Map[String, String]): DataFrame => DataFrame = df =>
    if (conf.isEmpty) df
    else {
      conf.foldLeft(df) {
        case (d, tuple) => d.withColumn(tuple._2, getJsonObjectNullSafe(col, s"$$.${tuple._1}"))
      }
    }

  override protected def invoke(p: ReaderConfig, spark: SQLContext): Unit = {
    val theUserAgent: UserAgentAnalyzer = userAgentFactory
    val ua: Broadcast[UserAgentAnalyzer] = spark.sparkContext.broadcast[UserAgentAnalyzer](theUserAgent)
